...
fn main() {
    ...
    let called = vec![
        31, 50, 79, 59, 39, 53, 58, 95, 92, 55, 40, 97, 81, 22, 69, 26, 6, 23, 3, 29, 83, 48, 18,
        75, 47, 49, 62, 45, 35, 34, 1, 88, 54, 16, 56, 77, 28, 94, 52, 15, 0, 87, 93, 90, 60, 67,
        68, 85, 80, 51, 20, 96, 61, 66, 63, 91, 8, 99, 70, 13, 71, 17, 7, 38, 44, 43, 5, 25, 72, 2,
        57, 33, 82, 78, 89, 21, 30, 11, 73, 84, 4, 46, 14, 19, 12, 10, 42, 32, 64, 98, 9, 74, 86,
        27, 24, 65, 37, 41, 76, 36,
    ];
    ...
    for ch in &input4.iter().chunks(25) {
        if let Some((moves_t, result_t)) =
            day4::find_bingo(ch.map(|n| *n).collect_vec(), called.clone())
        {
    ...
    input_5
        .clone()
        .into_iter()
        .for_each(|dis| board.mark_this_route(dis.0, dis.1, dis.2, dis.3));
    ...
    input_5
        .clone()
        .into_iter()
        .for_each(|dis| board.mark_this_route_diag(dis.0, dis.1, dis.2, dis.3));

    println!("day5 {}", board.count());

    // man, this main is starting to look like garbage indeed.
    let input_6 = read_lines_into("./files/day6.txt".to_string(), |number| {
        number
            .split_whitespace()
            .collect::<String>()
            .parse()
    ...
    // let mut day6_s = day6::School::new(input_6.clone());
    // day6_s = day6_s.add_days(80);
    // println!("day6.1 {:?}", day6_s.fishes());

    let day6_mo = day6_civilized::lanterns_with_cache(input_6, 256);
    println!("day6 {:?}", day6_mo);
}
